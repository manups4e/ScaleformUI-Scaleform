class com.rockstargames.PauseMenu.lobby.ImageColumnList
{
	var _MC;
	var _parentMC;
	...
	function ImageColumnList(parent, offset, id)
	{
		this._id = id;
		this.OFFSET = offset;
		this._minItem = 0;
		...
		this.BodyMask = this._MC.attachMovie("pauseMenuRightBody", "Body_store", this._MC.getNextHighestDepth());
		...
		this.Footer = this._MC.attachMovie("MenuFooter", "footer_store", this._MC.getNextHighestDepth());
	...
	function AddItem(before, index, id, param0, param1, param2, param3, param4, param5, param6, param7, param8, param9, param10, param11, param12, param13)
	{
		...
		if (id == 0)
		{
			item = new com.rockstargames.PauseMenu.lobby.StoreContentImageItem(this, index, param0, param1, param2, param3, param4, param5, param6, param7, param8, param9, param10, param11, param12, param13);
		}

		if (before)
		{
			item.itemMC._y = this.ItemList[0].itemMC._y - item.itemMC._height - 2;
		...
		else
		{
			...
			else
			{
				...
				item.itemMC._y = lastItem.itemMC._y + lastItem.itemMC._height + 2;
			}
	...
	function RemoveItem(id)
	{
		...
		this.itemCount = this.ItemList.length;
		this._itemsOffset = this.BodyMask._y + this.BodyMask.bgMC._height + 2;
		this.updateItemsDrawing();
	}
	...
	function updateItemsDrawing()
	{
		...
		{
			this.scrollableContent._y = this.MaxHeight - (this.currentItem.itemMC._y + this.currentItem.itemMC._height + 2);
		}
	...
	function updateDescription()
	{
		...
		{
			this.Footer = this._MC.attachMovie("MenuFooter", "footer_store_" + this._MC.getNextHighestDepth(), this._MC.getNextHighestDepth());
		}
		this.Footer._visible = this._isVisible && this.tot > this.MAX_ITEMS;
		this.Footer._x = this.OFFSET;// cambierà in base alle necessità
		this.Footer._y = this.MaxHeight;
	...
	function get MaxHeight()
	{
		...
		{
			_h += this.ItemList[i].itemMC._height + 2;
		}
		return _h;
	}
